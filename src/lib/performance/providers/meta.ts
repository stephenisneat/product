import type { MetaClient } from "@/lib/channels/providers/meta";
import type {
...
export const metaPerformanceProvider: PerformanceProvider = {
  provider: "meta",
  async fetchDailyCampaignMetrics(
    ctx: PerformanceFetchContext,
    range: { startDate: string; endDate: string },
  ): Promise<PerformanceFetchResult> {
    const client = ctx.credentials as MetaClient;
