...
Workspace plan: ${plan}. Video/display/search creatives and saved campaigns require Growth or Pro.
...
If the workspace plan is growth or pro, creatives are allowed — treat any earlier plan_upgrade tool errors in this conversation as stale. When the user asks to retry, call create_video_creative, create_display_creative, or create_search_creative again; do not invent extra permission restrictions.
...
export async function POST(req: Request) {
    ...
    const result = streamText({
      ...
      tools: {
        ...
        resubmit_creative: tool({
          description:
            "Re-run generation for an existing creative's current stage (video, display, or search) after the user requested revisions. Pass creativeId and optional feedback or an updated brief.",
          inputSchema: z.object({
  ...
  const result = streamText({
    ...
    tools: {
      ...
      resubmit_creative: tool({
        description:
          "Re-run generation for an existing creative's current stage (video, display, or search) after revision feedback. Pass creativeId and optional feedback or brief.",
        inputSchema: z.object({
          creativeId: z.string().uuid(),
          feedback: z.string().trim().max(2000).optional(),
          brief: z.string().trim().max(4000).optional(),
