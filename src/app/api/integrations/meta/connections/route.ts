...
/** List Meta Ads connections for the active workspace. */
export async function GET() {
  ...
  try {
    ...
    const meta = connections.filter((c) => c.provider === "meta");
    return NextResponse.json({
      metaConfigured: hasMetaConfig(),
      connections: meta,
      pending:
        meta.find((c) => c.status === "pending" && !c.externalAccountId) ?? null,
    });
  } catch (error) {
    const message =
...
/** Disconnect a Meta Ads connection. */
export async function DELETE(request: Request) {
  ...
  if (!canManageMembers(auth.active.role)) {
    return NextResponse.json(
      { error: "Only workspace owners and admins can disconnect Meta Ads." },
      { status: 403 },
    );
  }
