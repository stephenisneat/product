...
export async function GET(request: Request) {
  ...
  if (!active) {
    return NextResponse.redirect(`${settingsUrl}?meta=error&reason=no_workspace`);
  }
  ...
  if (oauthError) {
    return NextResponse.redirect(
      `${settingsUrl}?meta=error&reason=${encodeURIComponent(oauthError)}`,
    );
  }

  if (!code || !state) {
    return NextResponse.redirect(`${settingsUrl}?meta=error&reason=missing_params`);
  }

  const cookieStore = await cookies();
  const stateCookie = cookieStore.get(META_STATE_COOKIE)?.value;
  if (!stateCookie) {
    return NextResponse.redirect(`${settingsUrl}?meta=error&reason=missing_state`);
  }

  const [expectedState, expectedWorkspaceId] = stateCookie.split(".");
  if (expectedState !== state || expectedWorkspaceId !== active.workspace.id) {
    return NextResponse.redirect(`${settingsUrl}?meta=error&reason=state_mismatch`);
  }

  try {
    getMetaConfig();
  } catch {
    return NextResponse.redirect(`${settingsUrl}?meta=error&reason=not_configured`);
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      `${settingsUrl}?meta=error&reason=token_exchange`,
    );
  }

  const response = NextResponse.redirect(`${settingsUrl}?meta=select_account`);
