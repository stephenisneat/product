...
export async function GET(request: Request) {
  ...
  if (!user) {
    return NextResponse.redirect(`${appUrl}/login?error=auth&next=/settings/connections`);
  }

  const active = await getActiveWorkspace();
  if (!active) {
    return NextResponse.redirect(`${settingsUrl}?google_ads=error&reason=no_workspace`);
  }
  ...
  if (oauthError) {
    return NextResponse.redirect(
      `${settingsUrl}?google_ads=error&reason=${encodeURIComponent(oauthError)}`,
    );
  }

  if (!code || !state) {
    return NextResponse.redirect(`${settingsUrl}?google_ads=error&reason=missing_params`);
  }

  const cookieStore = await cookies();
  const stateCookie = cookieStore.get(GOOGLE_ADS_STATE_COOKIE)?.value;
  if (!stateCookie) {
    return NextResponse.redirect(`${settingsUrl}?google_ads=error&reason=missing_state`);
  }

  const [expectedState, expectedWorkspaceId] = stateCookie.split(".");
  if (expectedState !== state || expectedWorkspaceId !== active.workspace.id) {
    return NextResponse.redirect(`${settingsUrl}?google_ads=error&reason=state_mismatch`);
  }

  try {
    getGoogleAdsConfig();
  } catch {
    return NextResponse.redirect(`${settingsUrl}?google_ads=error&reason=not_configured`);
  }
  ...
  try {
    ...
    if (!token.refreshToken) {
      return NextResponse.redirect(
        `${settingsUrl}?google_ads=error&reason=no_refresh_token`,
      );
    }
    ...
    return NextResponse.redirect(
      `${settingsUrl}?google_ads=error&reason=token_exchange`,
    );
  }

  const response = NextResponse.redirect(
    `${settingsUrl}?google_ads=select_account`,
  );
  response.cookies.set(GOOGLE_ADS_STATE_COOKIE, "", {
    httpOnly: true,
    path: "/",
    maxAge: 0,
  });
