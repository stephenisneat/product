...
import { listAmazonProducts } from "@/lib/commerce/providers/amazon";
  ...
  try {
    ...
    if (!connection) {
      return NextResponse.json(
        { error: "No active Amazon connection. Connect a seller account first." },
        { status: 404 },
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : "Failed to list Amazon products";
