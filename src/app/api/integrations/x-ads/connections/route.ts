...
/** List X Ads connections for the active workspace. */
export async function GET() {
  ...
  try {
    ...
    const x = connections.filter((c) => c.provider === "x");
    return NextResponse.json({
      xAdsConfigured: hasXAdsConfig(),
      connections: x,
      pending:
        x.find((c) => c.status === "pending" && !c.externalAccountId) ?? null,
    });
  } catch (error) {
    const message =
...
/** Disconnect an X Ads connection. */
export async function DELETE(request: Request) {
  ...
  if (!canManageMembers(auth.active.role)) {
    return NextResponse.json(
      { error: "Only workspace owners and admins can disconnect X Ads." },
      { status: 403 },
    );
  }
